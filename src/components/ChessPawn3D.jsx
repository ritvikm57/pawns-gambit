...
function PawnRig() {
  ...
  // Read from sessionStorage on mount so we start at the correct scroll position
  // when navigating back to Home — same value useLayoutEffect uses to restore
  // window.scrollY, avoiding any race with the Three.js renderer.
  ...
  useFrame((_, delta) => {
    ...
    // S1 (sp < 0.5)  S2 (0.5 – 1.5)  S3 (sp ≥ 1.5)
    const X0      = Math.min(viewport.width * 0.4, 0)
    const targX   = sp < 0.5 ? X0 : sp < 1.5 ? -2 : -Math.min(viewport.width * 0.25, 3)
    const targTiltZ = sp < 0.5 ? 0 : sp < 1.5 ? -(Math.PI / 6) : (Math.PI / 12)
    const targOp  = 1
    // Past S3 the pawn exits upward; multiplier makes it vanish quickly
    ...
    // Float tied to tilt: full amplitude when upright (S1), zero when tilted (S2)
